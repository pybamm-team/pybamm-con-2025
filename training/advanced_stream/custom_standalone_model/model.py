...
class ThermalDFN(pybamm.lithium_ion.BaseModel):
    ...
    def __init__(self, name="Doyle-Fuller-Newman model"):
        ...
        # Temperature model
        ######################
        c_p_av = pybamm.x_average(pybamm.r_average(c_s_p / param.p.prim.c_max))
        c_n_av = pybamm.x_average(pybamm.r_average(c_s_n / param.n.prim.c_max))
        OCV = param.p.prim.U(c_p_av, T) - param.n.prim.U(c_n_av, T)
        Q_thermal = I * (OCV - voltage)
        R_c = pybamm.Parameter("R_c")
        C_c = pybamm.Parameter("C_c")
        self.rhs[T] = (Q_thermal - (T - param.T_init) / R_c) / C_c
